    ...
    <div class="content">
        <div class="container-fluid">
            <div class="page-header mb-4 text-center">
                ...
                <a href="{{ route('admin.viewActivitiesTopic', $assignment->id) }}" class="text-white">Back to Topics</a>
            </div>
                    ...
                    <form action="{{ route('admin.topic.update', [$assignment, $topic]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            @if($topic->file_path)
                                <div class="mb-2">
                                    @php
                                        $files = json_decode($topic->file_path, true) ?? [];
                                    @endphp
                                    @foreach($files as $file)
                                        <a href="{{ asset('storage/' . $file['path']) }}" target="_blank">{{ $file['filename'] }}</a>@if(!$loop->last), @endif
                                    @endforeach
                                </div>
                            @endif
                        ...
                        <a href="{{ route('admin.viewActivitiesTopic', $assignment->id) }}" class="btn btn-secondary ml-2">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
